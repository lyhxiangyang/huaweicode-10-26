...
def FeatureextractionData(inputDict: Dict, requestData: Dict = None):
    ...
    # extraction_ping_pds = predictpingpds
    extraction_topdown_pds = predicttopdwnpds
    return detectionJson, extraction_server_pds, extraction_process_pds, extraction_l2_pds, extraction_network_pds, extraction_ping_pds, extraction_topdown_pds
...
def saveoutputJsonFilename(inputDict: Dict, outputJsonDict):
    ...
    if inputDict["resultsavepath"] is None:
        return
    if not os.path.exists(inputDict["resultsavepath"]):
        os.makedirs(inputDict["resultsavepath"])
    if inputDict["outputJsonFilename"] is not None and len(inputDict["outputJsonFilename"]) != 0:
        outputJsonFilename = inputDict["outputJsonFilename"]
    saveDictToJson(outputJsonDict, inputDict["resultsavepath"], outputJsonFilename)
...
def detectionFromInputDict(inputDict: Dict, requestData: Dict = None) -> Dict:
    ...
    # =====================数据合并
    detectionJson = extractionpds[0]
    allserverpds = mergeDataFrames(extractionpds[1])
    allprocesspds = mergeDataFrames(extractionpds[2])
    alll2pds = mergeDataFrames(extractionpds[3])
    allnetworkpds = mergeDataFrames(extractionpds[4])
    allpingpds = mergeDataFrames(extractionpds[5])
