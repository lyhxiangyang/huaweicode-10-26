...
if __name__ == "__main__":
    ...
    # 其中单个server文件我默认是连续的
    # 指定异常数据的路径
    predictdirpath = R"C:\Users\lWX1084330\Desktop\正常和异常数据\训练数据-E5-1km-异常数据" # hjx1
    predictserverfiles = getfilespath(os.path.join(predictdirpath, "server"))
    # 指定正常server和process文件路径
    normaldirpath = R"C:\Users\lWX1084330\Desktop\正常和异常数据\E5-3km-正常数据" # hjx2
    normalserverfiles = getfilespath(os.path.join(normaldirpath, "server"))

    spath = "tmp/11-19-tData/训练数据-E5-1km/server" # hjx3

    # 需要对server数据进行处理的指标
    server_feature = ["mem_used", "pgfree"] # hjx4
    server_accumulate_feature = ["pgfree"] # hjx5

    # 在处理时间格式的时候使用，都被转化为'%Y-%m-%d %H:%M:00' 在这里默认所有的进程数据是同一种时间格式，
    server_time_format = '%Y/%m/%d %H:%M' # hjx6
