...
if __name__ == "__main__":
    ...
    # 其中单个server文件我默认是连续的
    predictdirpath = R"DATA\正常和异常数据\grapes数据\测试数据-Local-异常数据"
    predictserverfiles = getfilespath(os.path.join(predictdirpath, "server"))
    # 将一些需要保存的临时信息进行保存路径
    spath = "tmp/Grapes-Local-tmp"
    ...
    # 将used和pgfree修改为指定值
    normalserver_meanvalue["used"] = 27500000000
    normalserver_meanvalue["pgfree"] = 18000000
    ...
    max_depth = 5
    ModelTrainAndTest(allTrainedPD, allTestPD, spath=spath, selectedFeature=allfeatureload1_nosuffix, modelpath="tmp/grapemodels/memory_bandwidth_model-local", maxdepth=max_depth)
