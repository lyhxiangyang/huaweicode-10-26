...
if __name__ == "__main__":
    ...
    # 其中单个server文件我默认是连续的
    predictdirpath = R"DATA\正常和异常数据\测试数据-E5-1km-异常数据"
    ...
    predictL2files = getfilespath(os.path.join(predictdirpath, "L2"))
    predictNetworkfiles = getfilespath(os.path.join(predictdirpath, "network"))
    ...
    normalL2files = getfilespath(os.path.join(predictdirpath, "L2"))
    normalNetworkfiles = getfilespath(os.path.join(predictdirpath, "network"))
    # 预测CPU的模型路径
    ...
    # 温度过高异常
    temperatureIsTooHigh_modelpath = R"tmp/modelpath/l2/温度过高"
    # 网络异常PFC风暴注入
