...
def removeAbnormalPoints(df: pd.DataFrame, testLabelName: str = "preFlag") -> pd.DataFrame:
    ...
    def getRightLabel(x: pd.Series):
        ...
        if x.count(nowlable) == x.count(maxlabel):
            return nowlable
        return maxlabel
    df[testLabelName].rolling(window=winsize, center=True, min_periods=1).agg([getRightLabel])
    return df

def getTimePeriod(df: pd.DataFrame, timename: str = "time", realLabelName: str = "faultFlag", testLabelName: str = "preFlag") -> pd.DataFrame:
    tpd = getDetailedInformationOnTime(df, timeLabelName=timename, realFlagName=realLabelName, testFlagName=testLabelName)
